...
export async function emailPurchaseOrder(
  ...
  try {
    ...
    const { html, text } = renderLafloEmail({
      preheader: `Purchase order ${order.reference} is attached.`,
      title: `Purchase order ${order.reference}`,
      greeting: 'Hello,',
      intro: 'Attached is the purchase order. Please review the details.',
      meta: [
        { label: 'Reference', value: order.reference },
        { label: 'Vendor', value: order.vendorName || '-' },
        { label: 'Total', value: Number(order.totalCost).toFixed(2) },
      ],
