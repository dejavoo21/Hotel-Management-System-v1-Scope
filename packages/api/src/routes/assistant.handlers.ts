...
export type UnifiedAssistantMode = 'general' | 'operations' | 'pricing' | 'weather' | 'tasks';

type UnifiedAssistantChatBody = {
  message?: string;
  mode?: UnifiedAssistantMode;
  context?: Record<string, unknown> | null;
  conversationId?: string | null;
};

export function handleUnifiedAssistantStatus(_req: AuthenticatedRequest, res: Response) {
...
export async function handleUnifiedAssistantChat(
  ...
  try {
    ...
    const body = (req.body ?? {}) as UnifiedAssistantChatBody;
    const message = String(body.message ?? '').trim();
    const mode: UnifiedAssistantMode = body.mode ?? 'general';
    ...
    const reply = await generateUnifiedAssistantReply({
      hotelId,
      userId,
      message,
      mode,
...
async function generateUnifiedAssistantReply(args: {
  hotelId: string;
  userId: string;
  message: string;
  mode: UnifiedAssistantMode;
