...
export default function OperationsCenterPage() {
  ...
  const overviewRefreshButton = (
    <span className="inline-flex items-center gap-1.5">
      <span className="text-emerald-700">Auto-refresh: On</span>
      <button
        type="button"
        onClick={() => refreshWeatherMutation.mutate()}
        title={!canRefreshForecast ? "Forecast refresh requires operations permission" : "Refresh forecast"}
        aria-label="Refresh forecast"
        className="grid h-6 w-6 place-items-center rounded-md text-primary-700 transition hover:bg-primary-50 disabled:cursor-not-allowed disabled:opacity-50"
        disabled={refreshWeatherMutation.isPending || !canRefreshForecast}
      >
        <RefreshCcw className={`h-3.5 w-3.5 ${refreshWeatherMutation.isPending ? "animate-spin" : ""}`} />
      </button>
  ...
  const header = isOverview ? (
    <CommandHeader
      ...
      refreshButton={overviewRefreshButton}
    />
  ) : focus === "revenue" ? (
...
function CommandCenter({
  ...
  return (
    <div className="operations-command-center space-y-2 pb-28">
      {header}
...
function OperationsWorkspaceGrid({
  ...
  return (
    <>
      ...
      <section aria-label="Operations summary" className="operations-summary-grid grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
      ...
      <div className="operations-primary-grid grid gap-3 xl:grid-cols-[1.45fr_1fr_.9fr]">
        <section className="rounded-xl border border-border bg-card p-3 shadow-sm">
          ...
          <div className="operations-hourly-forecast mt-2 grid grid-cols-4 gap-2 sm:grid-cols-6 xl:grid-cols-12">
      ...
      <div className="operations-secondary-grid grid gap-3 xl:h-[198px] xl:grid-cols-[1.3fr_.72fr_1.05fr_1fr] xl:[&>section]:overflow-hidden xl:[&>section]:p-3">
      ...
      <div className="operations-footer-grid grid gap-3 xl:h-[150px] xl:grid-cols-[1fr_1.15fr_1fr] xl:[&>section]:overflow-hidden xl:[&>section]:p-3">
        ...
        <section className="rounded-xl border border-border bg-card p-4 shadow-sm"><div className="flex justify-between"><div><h2 className="text-sm font-semibold">Recommended Actions</h2><p className="text-[9px] text-text-muted">Authorised recommendations based on current operations.</p></div><Link to="/hotel-insights?tab=recommendations" className="text-[10px] font-semibold text-primary-700">Review</Link></div><div className="mt-2 grid gap-2 sm:grid-cols-3">{(briefing?.recommendedActions || []).slice(0, 3).map((item, index) => <Link key={`${item.title}-${index}`} to={canGovernance ? "/hotel-insights?tab=recommendations" : "#"} aria-disabled={!canGovernance} className={`rounded-lg border border-primary-100 bg-primary-50 p-2 ${!canGovernance ? "pointer-events-none opacity-50" : ""}`}><CheckCircle2 className="h-3.5 w-3.5 text-primary-700" /><strong className="mt-1 block line-clamp-2 text-[10px] leading-tight">{item.title}</strong><span className="mt-1 block line-clamp-2 text-[9px] leading-tight text-text-muted">{item.rationale}</span></Link>)}</div>{briefingLoading ? <p className="py-5 text-center text-xs text-text-muted">Loading recommendations…</p> : null}</section>
        <section className="rounded-xl border border-border bg-card p-4 shadow-sm"><h2 className="text-sm font-semibold">Quick Actions</h2><div className="mt-3 grid grid-cols-2 gap-2 xl:grid-cols-4"><QuickAction icon={ClipboardList} title="Assign task" detail="Open task queue" href="/operations/tasks-advisories?tab=tasks" restricted={!canTasks} /><QuickAction icon={AlertTriangle} title="Open incidents" detail="Review active" href="/incident-center?tab=active" /><QuickAction icon={BedDouble} title="Room readiness" detail="Integration status" href="/settings?tab=integrations" /><QuickAction icon={Gauge} title="Revenue guidance" detail="Review signal" href="/operations/operational-intelligence/revenue-guidance" restricted={!canRevenue} /></div></section>
      </div>
...
function CommandHeader({
  ...
  return (
    <header aria-label={isError ? "Operations context unavailable" : "Operations workspace live"} className="flex flex-col gap-3 px-1 py-1 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex items-start gap-3">
        <span className="theme-kpi-icon grid h-12 w-12 shrink-0 place-items-center rounded-full">
