    ...
    <>
      ...
      <section aria-label="Operations summary" className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7">
        {kpis.map(({ label, value, detail, icon: Icon, href, tone }) => {
          const body = (
            <article className={`h-full rounded-xl border border-border bg-card p-2.5 shadow-sm ${href ? "transition hover:border-primary-300 hover:shadow-md" : "opacity-75"}`}>
              <div className="flex items-start gap-3">
        ...
        <section className="rounded-xl border border-border bg-card p-3 shadow-sm">
          ...
          <div className="operations-hourly-forecast mt-2 grid grid-cols-4 gap-2 sm:grid-cols-6 2xl:grid-cols-12">
            {["Now", "12 PM", "2 PM", "4 PM", "6 PM", "8 PM", "10 PM", "12 AM", "2 AM", "4 AM", "6 AM", "8 AM"].map((time, index) => (
              <div key={time} className="rounded-lg border border-border bg-bg/60 p-1.5 text-center"><p className="text-[9px] font-bold text-text-main">{time}</p><ThermometerSun className={`mx-auto my-1 h-4 w-4 ${index === 0 ? "text-amber-500" : "text-slate-300"}`} /><p className="text-[10px] font-semibold">{index === 0 && context?.weather?.current?.temperatureC != null ? `${Math.round(context.weather.current.temperatureC)}°C` : "—"}</p><p className="mt-0.5 truncate text-[9px] text-text-muted">{index === 0 ? "Current" : "No hourly data"}</p></div>
            ))}
          </div>
      ...
      <div className="grid gap-3 xl:grid-cols-2 2xl:h-[198px] 2xl:grid-cols-[1.3fr_.72fr_1.05fr_1fr] 2xl:[&>section]:overflow-hidden 2xl:[&>section]:p-3">
      ...
      <div className="grid gap-3 2xl:h-[150px] 2xl:grid-cols-[1fr_1.15fr_1fr] 2xl:[&>section]:overflow-hidden 2xl:[&>section]:p-3">
