...
import {
  ...
  CloudRain, Gauge, House, RefreshCcw, Send, ShieldAlert, ShieldCheck, Sparkles, TrendingDown,
...
export default function OperationsCenterPage() {
  ...
  const refreshButton = <button type="button" onClick={() => refreshWeatherMutation.mutate()} className="btn-primary" disabled={refreshWeatherMutation.isPending}><RefreshCcw className={`h-4 w-4 ${refreshWeatherMutation.isPending ? 'animate-spin' : ''}`} />Refresh forecast</button>;
  ...
  return <div className="space-y-4">{header}{isOverview
...
function CommandCenter({ context, briefing, briefingLoading, pendingGovernance }: { context?: OperationsContext; briefing?: DailyGMBriefing; briefingLoading: boolean; pendingGovernance: number }) {
  ...
  return <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_400px]">
    ...
    <aside className="space-y-4">
      <SummaryPanel icon={Sparkles} title="Operations Concierge" description="AI-powered assistant for faster answers and smarter actions." action="Open concierge" href="/operations-center/ai" badge="AI connected"><p className="mb-2 text-xs font-semibold text-text-main">Suggested for you</p><div className="grid grid-cols-2 gap-2"><PromptLink text="What needs attention today?" /><PromptLink text="Summarize top advisories" /><PromptLink text="Which guests are at risk?" /><PromptLink text="Maintenance issues urgent?" /><PromptLink text="Housekeeping priorities" /><PromptLink text="Front desk guidance" /></div><Link to="/operations-center/ai" className="mt-3 flex items-center gap-2 rounded-xl border border-border bg-bg/30 p-2.5 text-xs text-text-muted"><span className="flex-1">Ask an operational question...</span><span className="grid h-8 w-8 place-items-center rounded-lg bg-slate-700 text-white"><Send className="h-4 w-4" /></span></Link></SummaryPanel>
...
function SummaryPanel({ icon: Icon, title, description, action, href, badge, children }: { icon: typeof Activity; title: string; description: string; action: string; href: string; badge?: string; children: React.ReactNode }) { return <section className="rounded-2xl border border-border bg-card p-4 shadow-sm"><div className="flex items-start gap-3"><span className="theme-kpi-icon grid h-10 w-10 shrink-0 place-items-center rounded-xl"><Icon className="h-5 w-5" /></span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><h2 className="font-semibold text-text-main">{title}</h2>{badge ? <span className="shrink-0 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-semibold text-emerald-700 ring-1 ring-emerald-200">{badge}</span> : null}</div><p className="mt-1 text-xs leading-5 text-text-muted">{description}</p></div></div><div className="mt-3">{children}</div><Link className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-primary-600" to={href}>{action} <ArrowRight className="h-3.5 w-3.5" /></Link></section>; }
function PromptLink({ text }: { text: string }) { return <Link to="/operations-center/ai" className="rounded-lg border border-border bg-bg/30 px-2.5 py-2 text-[11px] font-medium text-text-muted hover:border-primary-300 hover:text-primary-700">{text}</Link>; }
