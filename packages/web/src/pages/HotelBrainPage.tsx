    ...
    <div className="space-y-4">
      ...
      {incomingQuestion ? (
        <section className="flex flex-col gap-3 rounded-2xl border border-sky-200 bg-sky-50 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-xs font-semibold uppercase tracking-wide text-sky-700">Question ready for Ask LaFlo</p><p className="mt-1 text-sm font-medium text-text-main">{incomingQuestion}</p></div>
        ...
        <div className="space-y-4">
          <section className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            {briefingQuery.isLoading ? <div className="mt-4 h-32 animate-pulse rounded-xl bg-border/50" /> : briefing ? (
              <div className="mt-4">
                <p className="rounded-xl border border-border bg-bg p-4 text-sm leading-6 text-text-main">{briefing.executiveSummary}</p>
...
function ConsoleList({ icon: Icon, title, subtitle, items, empty, actionLabel, onAction, onItem }: { icon: typeof History; title: string; subtitle: string; items: string[]; empty: string; actionLabel?: string; onAction?: () => void; onItem?: (item: string) => void }) {
  return <section className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-start gap-3"><Icon className="mt-0.5 h-5 w-5 text-text-muted" /><div><h2 className="text-base font-semibold text-text-main">{title}</h2><p className="mt-1 text-xs leading-5 text-text-muted">{subtitle}</p></div></div><div className="mt-4 space-y-2">{items.length ? items.map((item) => onItem ? <button key={item} type="button" onClick={() => onItem(item)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-border px-3 py-2.5 text-left text-xs font-medium text-text-main hover:border-emerald-300 hover:bg-emerald-50/40"><span>{item}</span><ArrowRight className="h-3.5 w-3.5 shrink-0" /></button> : <div key={item} className="rounded-xl border border-border px-3 py-2.5 text-xs text-text-main">{item}</div>) : <EmptyState text={empty} />}</div>{actionLabel && onAction ? <button type="button" onClick={onAction} className="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-emerald-800">{actionLabel}<ArrowRight className="h-3.5 w-3.5" /></button> : null}</section>;
}

function AuditRow({ icon: Icon, label, value, onClick }: { icon: typeof ShieldCheck; label: string; value: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="flex w-full items-center gap-3 rounded-xl p-1 text-left hover:bg-bg"><span className="grid h-8 w-8 place-items-center rounded-lg bg-bg text-text-muted"><Icon className="h-4 w-4" /></span><span className="min-w-0 flex-1 text-xs font-medium text-text-muted">{label}</span><span className="text-xs font-semibold text-text-main">{value}</span></button>;
}
...
function EmptyState({ text }: { text: string }) {
  return <p className="rounded-xl border border-dashed border-border bg-bg p-4 text-xs text-text-muted">{text}</p>;
}
