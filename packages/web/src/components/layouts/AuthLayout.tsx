  ...
  return (
    <div className="min-h-dvh bg-white lg:grid lg:grid-cols-[minmax(0,55fr)_minmax(0,45fr)]">
      <aside className="relative hidden min-h-dvh overflow-x-hidden overflow-y-auto bg-[#dff4ea] lg:flex lg:flex-col" aria-label="LaFlo hotel operations overview">
        ...
        />
        <div
          className="absolute inset-0 bg-gradient-to-b from-white/10 via-[#e9f8f1]/5 to-[#087c66]/20"
          aria-hidden="true"
        />
        <div className="relative z-10 flex min-h-dvh flex-col px-10 py-6 xl:px-14 xl:py-8">
          ...
          <div className="relative mt-6 grid grid-cols-2 gap-3 xl:gap-4" aria-label="Hotel operations preview">
            {operationCards.map(({ label, value, detail, icon: Icon, tone }) => (
              <article key={label} className="rounded-2xl border border-white/70 bg-white/60 p-3.5 shadow-[0_16px_36px_rgba(12,91,72,0.12)] backdrop-blur-md transition hover:-translate-y-0.5 hover:bg-white/75 xl:p-4">
