  ...
  return <div className="space-y-4 pb-20">
    ...
    <div className="grid gap-4 xl:grid-cols-[250px_minmax(0,1fr)_320px]">
      <aside className="theme-card rounded-2xl border p-3"><div className="space-y-1">{navItems.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setTab(id)} className={`flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold ${tab === id ? 'bg-primary-50 text-primary-700' : 'text-text-muted hover:bg-bg'}`}><Icon className="h-4 w-4" />{label}</button>)}</div><div className="my-3 border-t border-border" />
        ...
        <h2 className="mt-4 px-1 text-xs font-bold uppercase tracking-wide text-text-muted">Quick actions</h2><div className="mt-2 space-y-1">{['Call Room Directory','Intercom / Departments','Voicemail','Emergency Contact List'].map((label) => <button key={label} type="button" onClick={() => openUnavailable(label, 'No live hotel directory or telephony endpoint is connected for this action.')} className="flex w-full items-center justify-between rounded-lg border border-border px-3 py-2.5 text-left text-xs font-semibold hover:bg-bg"><span className="flex items-center gap-2"><Building2 className="h-4 w-4 text-text-muted" />{label}</span><span aria-hidden>›</span></button>)}</div>
      </aside>
