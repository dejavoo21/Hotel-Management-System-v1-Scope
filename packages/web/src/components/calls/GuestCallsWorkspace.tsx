...
const guestPortraits: Record<string, string> = { 'michael chen': '/assets/guests/michael-chen.png', 'sarah johnson': '/assets/guests/sarah-johnson.png', 'john smith': '/assets/guests/john-smith.png' };
const guestPortrait = (guest: Pick<Guest, 'firstName' | 'lastName'>) => guestPortraits[`${guest.firstName} ${guest.lastName}`.toLowerCase()];
const keys = [['1',''],['2','ABC'],['3','DEF'],['4','GHI'],['5','JKL'],['6','MNO'],['7','PQRS'],['8','TUV'],['9','WXYZ'],['*',''],['0','+'],['#','']];
const normalize = (value: string) => {
  const leadingPlus = value.trimStart().startsWith('+');
  const digits = value.replace(/[^\d*#]/g, '');
  return `${leadingPlus ? '+' : ''}${digits}`.slice(0, 16);
};
...
export default function GuestCallsWorkspace() {
  ...
  useEffect(() => {
    if (!selectedId && guestsQuery.data?.data?.[0])
      setSelectedId(guestsQuery.data.data[0].id);
  }, [guestsQuery.data, selectedId]);
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if ((event.target as HTMLElement)?.matches('input, textarea, select')) return;
      if (/^[0-9*#]$/.test(event.key)) appendDialCharacter(event.key);
      if (event.key === 'Backspace') removeLastDialCharacter();
    };
  ...
  const selectedGuestRecents = useMemo(() => recents.filter((call) => call.guestId === selectedGuest?.id).slice(0, 3), [recents, selectedGuest?.id]);
  ...
  const kpis = [
    { label: 'Active Line', value: providerConnected ? '1' : '0', detail: providerLabel, icon: PhoneCall, tone: 'emerald', action: () => openUnavailable('Calling provider status', providerConnected ? `Connected${voice.data?.fromPhone ? ` from ${voice.data.fromPhone}` : ''}.` : 'Calling is not connected. Connect a provider in Integration Manager.') },
    { label: 'Missed Calls Today', value: recents.filter((item) => item.status === 'missed').length, detail: 'View missed calls', icon: PhoneMissed, tone: 'rose', action: () => { setFilter('missed'); setTab('recents'); } },
    { label: 'Recent Calls', value: recents.length, detail: 'Recorded on this device', icon: Clock3, tone: 'sky', action: () => setTab('recents') },
    { label: 'Connected Contacts', value: guestsQuery.data?.pagination.total || 0, detail: 'Guest profiles', icon: UsersRound, tone: 'emerald', action: () => setTab('contacts') },
    { label: 'Available Agents', value: agents.data?.filter((agent) => agent.online).length || 0, detail: agents.isError ? 'Availability unavailable' : 'Online now', icon: UserRoundCheck, tone: 'violet', action: () => openUnavailable('Agent availability', agents.isError ? 'Staff presence is unavailable.' : `${agents.data?.filter((agent) => agent.online).map((agent) => `${agent.firstName} ${agent.lastName}`).join(', ') || 'No agents are currently online.'}`) },
  ];
  ...
  return <div className="guest-calls-workspace -m-4 min-h-[calc(100dvh-4rem)] space-y-4 bg-slate-50/95 p-4 pb-20 lg:-m-5 lg:p-5 lg:pb-20">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[360px_repeat(4,minmax(0,1fr))]" aria-label="Call summary"><span aria-hidden="true" className="hidden xl:block" />{kpis.slice(0, 4).map(({ label, value, detail, icon: Icon, tone, action }) => <button key={label} type="button" onClick={action} className={`call-summary-card call-summary-card--${tone} theme-kpi min-w-0 rounded-2xl border p-3 text-left hover:border-primary-300`}><span className="flex min-w-0 items-start justify-between gap-2"><span className="min-w-0"><span className="block truncate text-xs font-semibold text-text-muted">{label}</span><strong className="mt-1 block text-xl text-text-main">{value}</strong></span><span className="call-summary-icon grid h-8 w-8 shrink-0 place-items-center rounded-xl"><Icon className="h-4 w-4" /></span></span><span className="mt-1 block truncate text-xs text-text-muted">{detail}</span></button>)}</section>
    <div className="grid gap-4 xl:grid-cols-[360px_minmax(560px,680px)_minmax(400px,1fr)]">
      <aside className="theme-card rounded-2xl border p-3 xl:-mt-[100px]"><div className="mb-4 flex items-start gap-3 border-b border-border p-2 pb-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-primary-50 text-primary-700"><PhoneCall className="h-5 w-5" /></span><div><h1 className="text-xl font-bold text-text-main">Guest Calls</h1><p className="mt-1 text-xs text-text-muted">Connect with guests quickly and efficiently.</p></div></div><div className="space-y-1">{navItems.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setTab(id)} className={`flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold ${tab === id ? 'bg-primary-50 text-primary-700' : 'text-text-muted hover:bg-bg'}`}><Icon className="h-4 w-4" />{label}</button>)}</div><div className="my-3 border-t border-border" />
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">{[kpis[0], kpis[1], kpis[2], kpis[4]].map(({ label, value, detail, icon: Icon, tone, action }) => <button key={label} type="button" onClick={action} className={`call-mini-card call-mini-card--${tone} rounded-xl border border-border bg-bg p-3 text-left`}><span className="flex items-start justify-between"><span><span className="block text-[10px] font-semibold text-text-muted">{label.replace(' Calls Today',' Today')}</span><strong className="mt-1 block text-lg">{value}</strong></span><Icon className="h-4 w-4" /></span><span className="mt-1 block text-[9px] text-text-muted">{detail}</span></button>)}</div>
