  ...
  return <div className="space-y-4 pb-20">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_17rem]">
      <div className="space-y-4">
        ...
        {grouped.map((group) => <section key={group.name} className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm"><header className="flex items-center justify-between border-b border-border px-4 py-3"><h3 className="font-semibold text-text-main">{group.name}</h3><span className="rounded-full bg-bg px-2.5 py-1 text-xs text-text-muted">{group.items.length} notifications</span></header><div className="divide-y divide-border">{group.items.map((item) => <div key={item.id} className="grid gap-3 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center"><div className="flex min-w-0 items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-primary-50 text-primary-700">{item.category === 'Booking & Front Desk' ? <CalendarCheck className="h-4 w-4" /> : item.category === 'Operations & Maintenance' ? <Wrench className="h-4 w-4" /> : item.category === 'Security & Smart Building' ? <ShieldAlert className="h-4 w-4" /> : item.category === 'Reports & Summaries' ? <FileChartColumn className="h-4 w-4" /> : item.category === 'Guest Experience' ? <Sparkles className="h-4 w-4" /> : <ClipboardList className="h-4 w-4" />}</span><div className="min-w-0"><p className="font-semibold text-text-main">{item.title}</p><p className="text-sm text-text-muted">{item.description}</p></div></div><div className="flex items-center gap-2"><span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${priorityStyles[item.priority]}`}>{item.priority}</span><span className="hidden text-xs text-text-muted lg:inline">{values.inAppChannel ? 'In-app' : ''}{values.inAppChannel && values.emailChannel ? ' + ' : ''}{values.emailChannel ? 'Email' : ''}</span></div><Toggle checked={Boolean(values[item.id])} disabled={!canEdit} label={`${item.title} notifications`} onChange={() => setValue(item.id, !values[item.id])} /></div>)}</div></section>)}
